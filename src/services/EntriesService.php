...
class EntriesService extends Component
{
        ...
        foreach ($results as $row) {
            ...
            if ($viewableEntry) {
                ...
                if ($result[$viewableId]['excerpt'] === "") {
                    ...
                    $fieldText = $this->getFieldText($entry, $row['fieldId'], $needle);
    ...
    private function getSearchIndexHits($normalizedNeedle, $siteId, $sectionIds)
    {
        ...
        if ( is_array( $sectionIds ) && ! empty( $sectionIds ) ) {
            $query->join('LEFT JOIN', '{{%entries}}', '{{%searchindex}}."elementId"={{%entries}}."id"' )
    ...
    private function getFieldText($entry, $fieldId, $needle)
    {
        $queryRow = (new Query())
            ->select(['handle', 'type'])
            ->from('{{%fields}}')
        ...
        if ($fieldType == 'Redactor') {
            return $this->htmlToText($fieldValue);
        } elseif ($fieldType == 'PlainText') {
            return $fieldValue;
        } elseif ($fieldType == 'craft\fields\Matrix') {
            foreach ($fieldValue->all() as $item) {
                $blockFields = $item->getFieldLayout()->getFields();
                $blockFieldsHandles = [];
                foreach ($blockFields as $blockField) {
                    $type = $blockField->getContentColumnType();
                    if ($type == 'text') {
                        $blockFieldsHandles[] = $blockField['handle'];
                    }
                }
                if ($blockFieldsHandles) {
                    foreach ($blockFieldsHandles as $blockFieldsHandle) {
                        if (is_a($item->$blockFieldsHandle, 'craft\redactor\FieldData')) {
                            $textForSearch = $item->$blockFieldsHandle->getParsedContent();
                        } else {
                            $textForSearch = $item->$blockFieldsHandle;
                        }
                        if ($textForSearch && (stripos($textForSearch, $needle) !== false)) {
                            return $this->htmlToText($textForSearch);
                        }
                    }
                }
            }
